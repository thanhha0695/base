<template>
    ...
    >
        <b-card-body>
            <b-card-title>User Permissions</b-card-title>
        </b-card-body>
 ...
 computed: {
   ...
   methods: {
     fetchTools() {
       ...
       const userId = paramRoute.userId
       callApi.get(`manage/users/${ userId }/permission`, {
         params: {
     ...
     savePermission() {
       ...
       callApi.put(`manage/users/${ userId }/permission/update`, { permissions: data }).then(response => {
       }).catch(err => {})
     },
     changeView(val, item) {
